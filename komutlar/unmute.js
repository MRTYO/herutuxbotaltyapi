...
module.exports.run = async (bot, message, args) => {
    ...
    if (!user.roles.find(`name`, "Blocker-Susturulmuş")) return message.channel.sendEmbed(new Discord.RichEmbed().setColor('RANDOM').setTitle('Kişi Mutelenmemiş'))
    ...
    let muterole = message.guild.roles.find(`name`, "Blocker-Susturulmuş");

    if (!muterole) {
        try {
           muterole = await message.guild.createRole({
                name: "Blocker-Susturulmuş",
                color: "#000000",
                permissions: []
            })
